...
function isInInterval(hour: number, start: number, end: number, displayedHours: number[]) {
...
const ClockTimeSelector: React.FC<ClockTimeSelectorProps> = ({
  ...
  // Получаем все часы в выбранном интервале
  function getSelectedHours(start: number, end: number): number[] {
    ...
    if (si <= ei) {
      // Обычный диапазон
      for (let i = si; i <= ei; i++) {
        selectedHours.push(displayedHours[i]);
      }
    } else {
      // Через полночь
      for (let i = si; i < displayedHours.length; i++) {
        selectedHours.push(displayedHours[i]);
      }
      for (let i = 0; i <= ei; i++) {
        selectedHours.push(displayedHours[i]);
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      >
        ...
        {/* Синие секторы выбранного интервала */}
        {showInterval &&
          getSelectedHours(start!, end!).map((h) => (
            <path
              key={`select-${h}`}
              d={describeArc(
                CENTER,
        ...
        {displayedHours.map((h) => {
          ...
          const isSelected = showInterval && getSelectedHours(start!, end!).includes(h);
          return (
            <g key={h}>
              <circle
                cx={x}
      ...
      <div className="mt-2 text-sm">
        {showInterval
          ? `${getLabel(start!)}:00 - ${getLabel(end!)}:00`
          : selection[0] !== null
